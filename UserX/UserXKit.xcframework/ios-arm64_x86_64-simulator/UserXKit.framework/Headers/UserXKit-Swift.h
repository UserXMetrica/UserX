...
/// warning:
/// <em>Never</em> change this property.<br/>
/// Consultation with the UserX team is required. Otherwise unexpected errors may occur.
SWIFT_CLASS_PROPERTY(@property (nonatomic, class) BOOL isMainThreadRendering;)
+ (BOOL)isMainThreadRendering SWIFT_WARN_UNUSED_RESULT;
+ (void)setIsMainThreadRendering:(BOOL)newValue;
...
///
/// \param isTriggerMode upload marked sessions only.
///
+ (void)start:(NSString * _Nonnull)apiKey uploadIfMarked:(BOOL)isTriggerMode;
...
+ (void)startGesturesAttribution;
+ (void)stopGesturesAttribution;
+ (void)clearSessions;
@end
